...
function scheduleAutoClean() {
  ...
  if (storageConfig.autoCleanEnabled) {
    const days =
      storageConfig.autoCleanDays || DEFAULT_STORAGE_CONFIG.autoCleanDays;
...
ipcMain.handle("save-image", async (_event, { url, category, filename }) => {
  try {
    ...
    return {
      success: true,
      localPath: `local-image://${category}/${safeName}`,
    };
  } catch (error) {
...
ipcMain.handle(
  ...
  async (_event, key: string, value: string) => {
    try {
      ...
      console.log(
        `Saved to file: ${filePath} (${Math.round(value.length / 1024)}KB)`,
      );
      return true;
    } catch (error) {
  ...
  try {
    ...
    if (fs.existsSync(projectsDir)) {
      ...
      if (fs.existsSync(perProjectDir)) {
        const projectDirs = await fs.promises.readdir(perProjectDir, {
          withFileTypes: true,
        });
        const dirCount = projectDirs.filter(
...
ipcMain.handle("storage-import-data", async (_event, sourcePath: string) => {
  try {
    ...
    if (!hasProjects && !hasMedia) {
      return {
        success: false,
        error: "源目录不包含有效数据（需要 projects/ 或 media/ 子目录）",
      };
    }
...
ipcMain.handle(
  "storage-clear-cache",
  async (_event, options?: { olderThanDays?: number }) => {
    try {
...
ipcMain.handle(
  ...
  async (
    _event,
    config: { autoCleanEnabled?: boolean; autoCleanDays?: number },
  ) => {
